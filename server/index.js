  ...
  try {
    ...
    data.data.forEach(obj=> {
      const item = {
        ...items[obj.name],
        gridPosition: [
          obj.position.horizontal_position,
          obj.position.vertical_position
        ],
      };
      if (obj.rotation) {
    ...
    console.error("Failed to create map items:", error);
    return [];
  }
}

const modifiedItems = await createMapItems(items_addition)
